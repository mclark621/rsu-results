  ...
  Future<void> _startOAuth() async {
    ...
    try {
      ...
      if (effectiveRedirect.isEmpty) {
        throw StateError(
            'Missing redirect_uri. Configure Firestore doc public_config/rsu (oauth_redirect_uri).');
      }
      ...
      if (kIsWeb) {
        ...
        if (configuredOrigin.isEmpty)
          throw StateError('Invalid redirect_uri: "$effectiveRedirect"');
      ...
      debugPrint(
          'OAuth redirect_uri="$effectiveRedirect" on origin="${Uri.base.origin}"');
        ...
        child: Column(
          ...
          children: [
            ...
            FilledButton.icon(
              ...
              label: Text(_loading ? 'Opening…' : 'Sign in with RunSignup'),
            ),
            const SizedBox(height: AppSpacing.md),
            if (isLoggedIn) ...[
              OutlinedButton.icon(
                ...
                label: Text('Global Settings (Timer API)',
